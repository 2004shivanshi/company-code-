...
const HalfyearlyUPIPage = () => {
  ...
  const handleUPIVerifyAndProceed =  (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      const url="https://financeshastra-backendupdated.onrender.com/api/paymentDetails1"
      const options= {
        method: "post",
        headers: {
      ...
      setTimeout(() => {
       ...
       // Popup duration set to 5 seconds
    }
  };
  ...
  const handleCardClick = () => {
    navigate("/halfyearlySubscriptionPages"); // Navigate to /billingDetailsPage
  };

  // Handle navigation on PayPal button click
  const handlePayPalClick = () => {
    navigate("/halfyearlyPaypalProfilePage"); // Navigate to /PaypalProfilePage
  };
  const handleupiClick = () => {
    navigate("/halfyearlyUPIPage"); // Navigate to /PaypalProfilePage
  };
  const handleScanAndPayProfilePage = () => {
    navigate("/halfyearlyScanPage");
  };
  return (
...
export default HalfyearlyUPIPage;
